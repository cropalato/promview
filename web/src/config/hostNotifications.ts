...
export function createHostNotificationFactory(invoke: Invoke): NotificationFactoryLike {
  return {
    ...
    create: (title: string, options: ShowNotificationOptions): NotificationHandle => {
      ...
      void Promise.resolve(invoke('show_notification', { title, body: options.body ?? '' })).catch(
        () => {
          // A notification that fails to appear must not take the stream
          // handler down with it; the alert is in the console either way.
        },
      );
